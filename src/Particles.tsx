...
import { openingProgress, type OpeningSequence } from './useOpeningSequence';

export default function Particles({ opening }: { opening: RefObject<OpeningSequence> }) {
  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      const entrance = reduced.matches ? 1 : openingProgress(opening.current, now);
      renderer.draw({ time: reduced.matches ? 0 : current.time, scroll: reduced.matches ? 0 : current.scroll,
        reading: Math.max(0, window.scrollY / height), entrance });
      // Pausing stops the grains immediately; the brief page reveal finishes
      // with the typography so resuming cannot jump the light to full strength.
      if ((!still || entrance < 1) && !document.hidden) frame = requestAnimationFrame(draw);
    };
    const restart = () => {
    ...
    const resize = () => {
      ...
      const dpr = Math.min(devicePixelRatio || 1, 2, Math.sqrt(4000000 / (width * height)));
    ...
    const onScroll = () => { if (pausedRef.current || reduced.matches) restart(); };
    const lost = (event: Event) => {
      event.preventDefault(); cancelAnimationFrame(frame);
      renderer?.dispose(); renderer = undefined; setAvailable(false);
    };
